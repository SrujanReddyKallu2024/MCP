...
def rename_files(
    ...
    try:
        for i, s3_uri in enumerate(all_files):
            ...
            logging.info(f"Copying {key} â†’ {new_key}")
            s3.copy_object(
...
def quick_stats(stats: Dict, stats_name: Dict, stats_prev: Dict, result: str) -> Tuple[Dict, Dict, str]:
    ...
    for key, value in stats_perc.items():
        if stats_perc[key] == '-.--':
            stats_tl[key] = "âšª"  # Neutral when no previous data
        elif isinstance(value, (int, float)):
            if value > 0:
                stats_tl[key] = "ðŸŸ¢"  # Positive change
            elif value < 0:
                stats_tl[key] = "ðŸ”´"  # Negative change
            else:
                stats_tl[key] = "âšª"  # No change
